...
The fourth is the actual optimization: `ExecutorPrepAndLock()`, the pruning-aware path in `PortalLockCachedPlan()`, the `firstResultRels` tracking for writable CTEs, and the regression tests. Multi-statement `CachedPlan`s always use conservative locking, for the same reasons as before.
